<template>
  <div class="app-container">
    <el-form inline>
      <!-- 表单元素 -->
      <el-form-item>
         <el-input v-model="tempSearchObj.username" placeholder="人物名称" />
      </el-form-item>
      <!-- 查询与情况的按钮 -->
    ...
    <el-table
      ...
      <el-table-column prop="username" label="人物" width="150" />
      <el-table-column prop="roleName" label="权限" />
      <!-- <el-table-column prop="roleName" label="二级目录" />
      <el-table-column prop="roleName" label="一级目录" /> -->
      ...
      <el-table-column label="操作" width="230" align="center">
        ...
        <template slot-scope="{row}">
          <HintButton type="info" size="mini" icon="el-icon-user-solid" title="修改人物数据"
            @click="showAssignRole(row)"/>
          <HintButton type="primary" size="mini" icon="el-icon-edit" title="修改人物信息"
    ...
    <el-pagination
      ...
      :total="total"
      :page-size="limit"
      :page-sizes="[3, 7,10, 20, 30, 40, 50, 100]"
    ...
    />
    <!-- 对话框的结构 -->
    <el-dialog :title="user.id ? '修改用户信息' : '添加用户'" :visible.sync="dialogUserVisible">
      <el-form ref="userForm" :model="user" :rules="userRules" label-width="120px">
        <el-form-item label="人物名称" prop="username">
    ...
    <el-dialog title="设置人物" :visible.sync="dialogRoleVisible" :before-close="resetRoleData">
  ...
  methods: {
    ...
    */
    async assignRole () {
      ...
      this.loading = false
      this.$message.success(result.message || '分配人物权限成功')
      this.resetRoleData()
    ...
    */
    revomveUsers () {
      this.$confirm('确定删除吗?').then(
        async () => {
        await this.$API.user.removeUser(this.selectedIds)
